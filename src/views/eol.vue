    ...
    <div class="content">
      ...
      <div class="content_right_box">
        ...
        <div class="content_cen">
          <div class="module_box contnent_cen_top" style="  overflow: hidden;">
            <div class="title" >单体检测信息</div>
            <div class="info_box params_list_box body" id="wrap" style="overflow: hidden;">
              <!-- <p class="sub_title">单体信息</p> -->
              <ul class="list"  >
                <li  class="params_list" v-for="item in this.singleList1" :key="item.singleId">
                  <div class="params_item" v-for="v in item" :key="v.singleId">
                ...
                <li  class="params_list" v-for="item in this.singleList1" :key="item.singleId+1">
                  <div class="params_item" v-for="v in item" :key="(v.singleId+2)">
                    <div class="params_item_title">{{ v.singleName }}</div>
                    <div class="params_item_small">电压:{{ v.singleVolt }}</div>
                    <div class="params_item_small">温度:{{ v.singleTemp }}</div>
                  </div>
...
export default {
  ...
  async mounted() {
    ...
    await eolList().then((res) => {
      ...
      for (let i=1;i*4<=res.data.length;i++){
        i ===1?this.singleList1.push(res.data.slice(0,4)):this.singleList1.push(res.data.slice((i-1)*4,i*4))
      }
      let { data } = res;
      data.forEach((item) => {
